    ...
    def get_game_state_reward(self, print_stats=False):
        ...
        if self.curriculum_stage == 1:
            state_scores = {
                ...
                'explore': self.reward_scale * self.get_knn_reward() * 1 # Focus on exploration
            }
            ...
            state_scores = {
                'event': self.reward_scale*self.update_max_event_rew(),
                #'party_xp': self.reward_scale*0.1*sum(poke_xps),
                'level': self.reward_scale*self.get_levels_reward() * 2, # Focus on catching pokemons
                'heal': self.reward_scale*self.total_healing_rew,
                'op_lvl': self.reward_scale*self.update_max_op_level() * 2,
                'dead': self.reward_scale*-0.1*self.died_count,
